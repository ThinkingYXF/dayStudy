...
+ function($) {
  ...
  $.modal = function(params, onOpen) {
    ...
    dialog.find(".weui-dialog__btn").each(function(i, e) {
      var el = $(e);
      el.click(function() {
        //å…ˆå…³é—­å¯¹è¯æ¡†ï¼Œå†è°ƒç”¨å›žè°ƒå‡½æ•°
  ...
  //å¦‚æžœå‚æ•°è¿‡å¤šï¼Œå»ºè®®é€šè¿‡ config å¯¹è±¡è¿›è¡Œé…ç½®ï¼Œè€Œä¸æ˜¯ä¼ å…¥å¤šä¸ªå‚æ•°ã€‚
  ...
  $.login = function(text, title, onOK, onCancel, username, password) {
    ...
    var modal = $.modal({
      text: '<p class="weui-prompt-text">'+(config.text || '')+'</p>' +
            '<input type="text" class="weui-input weui-prompt-input" id="weui-prompt-username" value="' + (config.username || '') + '" placeholder="è¾“å…¥ç”¨æˆ·å" />' +
            '<input type="password" class="weui-input weui-prompt-input" id="weui-prompt-password" value="' + (config.password || '') + '" placeholder="è¾“å…¥å¯†ç " />',
      title: config.title,
      autoClose: false,
      buttons: [
      {
  ...
  defaults = $.modal.prototype.defaults = {
    ...
    autoClose: true //ç‚¹å‡»æŒ‰é’®è‡ªåŠ¨å…³é—­å¯¹è¯æ¡†ï¼Œå¦‚æžœä½ ä¸å¸Œæœ›ç‚¹å‡»æŒ‰é’®å°±å…³é—­å¯¹è¯æ¡†ï¼Œå¯ä»¥æŠŠè¿™ä¸ªè®¾ç½®ä¸ºfalse
  };
...
+ function($) {
  ...
  $.toast = function(text, style, callback) {
    ...
    show('<i class="' + iconClassName + ' weui-icon_toast"></i><p class="weui-toast_content">' + (text || "å·²ç»å®Œæˆ") + '</p>', className);
  ...
  $.showLoading = function(text) {
    ...
    html += '</div>';
    html += '<p class="weui-toast_content">' + (text || "æ•°æ®åŠ è½½ä¸­") + '</p>';
  ...
  var show = function(params) {
    ...
    var tpl = '<div class="weui-actionsheet " id="weui-actionsheet">'+
                ...
                '<div class="weui-actionsheet__action">'+
                  '<div class="weui-actionsheet__cell weui-actionsheet_cancel">å–æ¶ˆ</div>'+
                  '</div>'+
                '</div>';
    var dialog = $(tpl).appendTo(document.body);
...
+function ($) {
  ...
  PTR.prototype.touchMove= function(e) {
    ...
    if (Math.abs(this.diffX) > Math.abs(this.diffY)) return true; // è¯´æ˜Žæ˜¯å·¦å³æ–¹å‘çš„æ‹–åŠ¨
...
+ function($) {
  ...
  //ä¿®å¤pickerä¼šæ»šåŠ¨é¡µé¢çš„bug
  ...
  $.openPicker = function(tpl, className, callback) {
    ...
    //å…³äºŽå¸ƒå±€çš„é—®é¢˜ï¼Œå¦‚æžœç›´æŽ¥æ”¾åœ¨bodyä¸Šï¼Œåˆ™åšåŠ¨ç”»çš„æ—¶å€™ä¼šæ’‘å¼€bodyé«˜åº¦è€Œå¯¼è‡´æ»šåŠ¨æ¡å˜åŒ–ã€‚
    var dialog = $(tpl).appendTo(container);

    dialog.width(); //é€šè¿‡å–ä¸€æ¬¡CSSå€¼ï¼Œå¼ºåˆ¶æµè§ˆå™¨ä¸èƒ½æŠŠä¸Šä¸‹ä¸¤è¡Œä»£ç åˆå¹¶æ‰§è¡Œï¼Œå› ä¸ºåˆå¹¶ä¹‹åŽä¼šå¯¼è‡´æ— æ³•å‡ºçŽ°åŠ¨ç”»ã€‚
...
+ function($) {
  ...
  Select.prototype.parseInitValue = function() {
    ...
    //å¦‚æžœinputä¸ºç©ºï¼Œåªæœ‰åœ¨ç¬¬ä¸€æ¬¡åˆå§‹åŒ–çš„æ—¶å€™æ‰ä¿ç•™é»˜è®¤é€‰æ‹©ã€‚å› ä¸ºåŽæ¥å°±æ˜¯ç”¨æˆ·è‡ªå·±å–æ¶ˆäº†å…¨éƒ¨é€‰æ‹©ï¼Œä¸èƒ½å†ä¸ºä»–é€‰ä¸­é»˜è®¤å€¼ã€‚
  ...
  Select.prototype.open = function(values, titles) {
    ...
    // open picker ä¼šé»˜è®¤å…³æŽ‰å…¶ä»–çš„ï¼Œä½†æ˜¯ onClose ä¸ä¼šè¢«è°ƒç”¨ï¼Œæ‰€ä»¥è¿™é‡Œå…ˆå…³æŽ‰å…¶ä»–select
    for (var i = 0; i < selects.length; i++ ) {
      var s = selects[i];
      if (s === this) continue;
      if (s._open) {
        if(!s.close()) return false; // å…¶ä»–çš„selectç”±äºŽæŸäº›æ¡ä»¶é™åˆ¶å…³é—­å¤±è´¥ã€‚
      }
    }
    ...
    if(!force) {
      ...
      if(this.config.multi) {
        if(this.config.min !== undefined && this.data.length < this.config.min) {
          $.toast("è¯·è‡³å°‘é€‰æ‹©"+this.config.min+"ä¸ª", "text");
          return false
        }
        if(this.config.max !== undefined && this.data.length > this.config.max) {
          $.toast("æœ€å¤šåªèƒ½é€‰æ‹©"+this.config.max+"ä¸ª", "text");
          return false
        }
      }
    }
  ...
  defaults = $.fn.select.prototype.defaults = {
    items: [],
    input: undefined, //è¾“å…¥æ¡†çš„åˆå§‹å€¼
    title: "è¯·é€‰æ‹©",
    multi: false,
    closeText: "ç¡®å®š",
    autoClose: true, //æ˜¯å¦é€‰æ‹©å®ŒæˆåŽè‡ªåŠ¨å…³é—­ï¼Œåªæœ‰å•é€‰æ¨¡å¼ä¸‹æ‰æœ‰æ•ˆ
    onChange: undefined, //function
    beforeClose: undefined, // function å…³é—­ä¹‹å‰ï¼Œå¦‚æžœè¿”å›žfalseåˆ™é˜»æ­¢å…³é—­
    onClose: undefined, //function
    onOpen: undefined, //function
    split: ",",  //å¤šé€‰æ¨¡å¼ä¸‹çš„åˆ†éš”ç¬¦
    min: undefined, //å¤šé€‰æ¨¡å¼ä¸‹å¯ç”¨ï¼Œæœ€å°‘é€‰æ‹©æ•°
    max: undefined, //å•é€‰æ¨¡å¼ä¸‹å¯ç”¨ï¼Œæœ€å¤šé€‰æ‹©æ•°
    toolbarTemplate: '<div class="toolbar">\
      <div class="toolbar-inner">\
...
+function ($) {
  ...
  var Calendar = function (params) {
      ...
      //iphone ä¸Šæ— æ³•æ­£ç¡®è§¦å‘ clickï¼Œä¼šå¯¼è‡´ç‚¹å‡»å¤–é¢æ— æ³•å…³é—­
      ...
      p.close = function () {
          if (!p.opened || p.inline) return;
          p.animating = false;  //æœ‰å¯èƒ½è¿˜æœ‰åŠ¨ç”»æ²¡åšå®Œï¼Œå› æ­¤animatingè®¾ç½®è¿˜æ²¡æ”¹ã€‚
          if (inPopover()) {
              $.closePicker(p.popover);
              return;
          }
  ...
  defaults = $.fn.calendar.prototype.defaults = {
    value: undefined, // é€šè¿‡JSèµ‹å€¼ï¼Œæ³¨æ„æ˜¯æ•°ç»„
...
+ function($) {
  ...
  defaults = $.fn.datetimePicker.prototype.defaults = {
    input: undefined, // é»˜è®¤å€¼
    min: undefined, // YYYY-MM-DD æœ€å¤§æœ€å°å€¼åªæ¯”è¾ƒå¹´æœˆæ—¥ï¼Œä¸æ¯”è¾ƒæ—¶åˆ†ç§’
    max: undefined,  // YYYY-MM-DD
    yearSplit: '-',
    monthSplit: '-',
    dateSplit: '',  // é»˜è®¤ä¸ºç©º
    datetimeSplit: ' ',  // æ—¥æœŸå’Œæ—¶é—´ä¹‹é—´çš„åˆ†éš”ç¬¦ï¼Œä¸å¯ä¸ºç©º
    ...
    format: function (p, values) { // æ•°ç»„è½¬æ¢æˆå­—ç¬¦ä¸²
      return p.cols.map(function (col) {
        return col.value || col.content;
      }).join('');
    },
    parse: function (str) {
      // æŠŠå­—ç¬¦ä¸²è½¬æ¢æˆæ•°ç»„ï¼Œç”¨æ¥è§£æžåˆå§‹å€¼
      // å¦‚æžœä½ çš„å®šåˆ¶çš„åˆå§‹å€¼æ ¼å¼æ— æ³•è¢«è¿™ä¸ªé»˜è®¤å‡½æ•°è§£æžï¼Œè¯·è‡ªå®šä¹‰è¿™ä¸ªå‡½æ•°ã€‚æ¯”å¦‚ä½ çš„æ—¶é—´æ˜¯ 'å­æ—¶' é‚£ä¹ˆé»˜è®¤æƒ…å†µè¿™ä¸ª'æ—¶'ä¼šè¢«å½“åšåˆ†éš”ç¬¦è€Œå¯¼è‡´é”™è¯¯ï¼Œæ‰€ä»¥ä½ éœ€è¦è‡ªå·±å®šä¹‰parseå‡½æ•°
      // é»˜è®¤å…¼å®¹çš„åˆ†éš”ç¬¦
      var t = str.split(this.datetimeSplit);
      return t[0].split(/\D/).concat(t[1].split(/:|æ—¶|åˆ†|ç§’/)).filter(function (d) {
        return !!d;
      })
    }
  }
...
+ function($) {
  ...
  //Popup å’Œ picker ä¹‹ç±»çš„ä¸è¦å…±ç”¨ä¸€ä¸ªå¼¹å‡ºæ–¹æ³•ï¼Œå› ä¸ºè¿™æ ·ä¼šå¯¼è‡´ åœ¨ popup ä¸­å†å¼¹å‡º picker çš„æ—¶å€™ä¼šæœ‰é—®é¢˜ã€‚
...
+function ($) {
  ...
  Swipeout.prototype.touchMove= function(e) {
    ...
    this.diffX = p.x - this.start.x;
    this.diffY = p.y - this.start.y;
    if (Math.abs(this.diffX) < Math.abs(this.diffY)) { // è¯´æ˜Žæ˜¯ä¸Šä¸‹æ–¹å‘åœ¨æ‹–åŠ¨
      this.close()
      this.start = false
      return true;
    }
  ...
  Swipeout.prototype.touchEnd = function() {
    ...
    if (this.diffX < -5 && t < 200) { // å‘å·¦å¿«é€Ÿæ»‘åŠ¨ï¼Œåˆ™æ‰“å¼€
      this.open()
    } else if (this.diffX >= 0 && t < 200) { // å‘å³å¿«é€Ÿæ»‘åŠ¨ï¼Œæˆ–è€…å•å‡»,åˆ™å…³é—­
      this.close()
